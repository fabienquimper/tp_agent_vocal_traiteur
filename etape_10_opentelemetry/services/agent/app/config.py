...
class Settings(BaseSettings):
    ...
    rag_quality_threshold: float = 0.8
    eval_dir: str = "/app/eval"

    # ── Environnement (étape 06) ───────────────────────────────────────────────
    # Identifiant d'environnement — injecté par docker-compose.staging.yml.
    # Visible dans GET /health → permet aux tests de vérifier l'environnement cible.
    # Ne change pas le comportement fonctionnel (uniquement logging + observabilité).
