...
def search_chunks(query: str, k: int = 3) -> list[dict]:
    ...
    Instrumentation OTel (étape 10) :
      Span "rag.similarity_search" avec attributs :
        - rag.query      → la requête (tronquée à 100 chars)
        - rag.k          → nombre de chunks demandés
        - rag.hits       → nombre de chunks retournés
        - rag.collection → nom de la collection active
    """
    if _vectorstore is None:
        initialize_vectorstore()

    tracer = otel_trace.get_tracer("traiteur-rag")
    with tracer.start_as_current_span("rag.similarity_search") as span:
        ...
        span.set_attribute("rag.collection", _status.active_collection or "unknown")
        ...
        try:
            ...
            span.set_attribute("rag.hits", len(chunks))
            return chunks
        except Exception as exc:
