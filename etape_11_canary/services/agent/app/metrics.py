...
#   - "Les quality gates passent-elles ?" (rag_quality_gate_total)
#   - "Quel est le taux de hit au moment du check ?" (rag_quality_gate_hit_rate)
#
# Pourquoi une quality gate ?
#   Sans vérification, un rebuild peut produire un index valide (pas d'erreur)
#   mais dégradé : documents mal encodés, embeddings corrompus, données tronquées.
#   La quality gate rejette ces cas silencieusement avant le swap en production.

rag_quality_gate_total = Counter(
    "rag_quality_gate_total",
    "Nombre de quality gates évaluées après un rebuild shadow",
    ["result"],  # "passed" | "failed"
)
rag_quality_gate_hit_rate = Histogram(
    "rag_quality_gate_hit_rate",
    "Taux de hit du RAG lors du contrôle qualité (0.0 = 0%, 1.0 = 100%)",
