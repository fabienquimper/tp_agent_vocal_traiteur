...
Endpoints :
  ...
  — Quality Gate + Hot RAG Reload (étape 05) ———————————
  POST /api/rag/rebuild      → démarre un rebuild shadow (non-bloquant, < 1ms)
                               Nouveau : quality gate exécutée avant l'atomic swap.
                               Si hit_rate < threshold → state="quality_gate_failed"
  GET  /api/rag/status       → état + résultat quality gate
                               state: idle|rebuilding|done|quality_gate_failed|error
  POST /api/rag/rollback     → revient à la collection précédente
  GET  /api/rag/search       → recherche sémantique brute (sans LLM) — NOUVEAU

  — Compatibilité étape 03 ——————————————————————————————
  POST /api/reload-documents → alias vers /api/rag/rebuild (non-bloquant)
...
async def lifespan(app: FastAPI):
    # ── OpenTelemetry — doit être initialisé AVANT les premières requêtes ──────
    # On passe `app` pour que FastAPIInstrumentor puisse s'accrocher aux routes.
    setup_telemetry(app)
...
async def _process_request(
    ...
    Instrumentation OTel (étape 10) :
      - FastAPIInstrumentor crée le span parent "POST /api/voice" (automatique)
      - Ce span est un child span de ce span parent
      - Les appels httpx (STT, TTS, Ollama) créent automatiquement des child spans
      - Le span "rag.retrieve" est créé manuellement pour avoir des attributs métier
    """
    ...
    if session_id:
        ...
        if session and session.step not in ("complete",):
            with tracer.start_as_current_span("session.handle_step") as span:
                span.set_attribute("session.id", session_id)
                span.set_attribute("session.step", session.step)
    ...
    try:
        # ── Span : graph.invoke (LangGraph → LLM classify + RAG retrieve) ────
        # Ce span englobe l'appel complet au graphe LangGraph.
        # Les httpx vers Ollama sont des child spans créés automatiquement.
        with tracer.start_as_current_span("langgraph.invoke") as span:
            span.set_attribute("llm.model", "mistral")
            span.set_attribute("input.channel", channel)
            span.set_attribute("input.text_length", len(text_input))
            result = await asyncio.to_thread(graph.invoke, initial_state)
            span.set_attribute("output.intent", result.get("intent", ""))
            span.set_attribute("output.rag_context_length", len(result.get("rag_context", "")))
    except Exception as exc:
        duration = time.perf_counter() - t0
        conversation_duration_seconds.observe(duration)
...
async def reload_documents():
    ...
    CHANGEMENT vs étape 03 :
      ...
      Maintenant : retourne immédiatement, rebuild en background + quality gate
    """
...
@app.get(
    "/api/rag/search",
    summary="Recherche sémantique brute dans le RAG",
    description=(
        "Effectue une recherche de similarité dans ChromaDB sans passer par le LLM. "
        "Utile pour déboguer la qualité de l'index, vérifier qu'un document est bien "
        "indexé, et pour les tests automatisés (rapide, ne nécessite pas Ollama)."
    ),
    tags=["RAG Hot Reload"],
)
async def rag_search(q: str, k: int = 3):
    """
    Recherche les k chunks les plus proches de la requête q.

    Retourne les chunks bruts avec leur score de similarité.
    Score proche de 0 = très similaire, score élevé = peu similaire.

    Utilisé par les tests automatisés (test_rag_quality.py) :
      - Rapide (pas de LLM)
      - Déterministe
      - Permet de calculer le hit rate sans Ollama
    """
    if not q.strip():
        raise HTTPException(status_code=400, detail="Le paramètre 'q' ne peut pas être vide")
    chunks = search_chunks(q.strip(), k=min(k, 10))
    return {
        "query": q,
        "k": k,
        "hits": len(chunks),
